...
k.list <- c(1,2,3,4,5,6,7,8,9,15,20);
#k.list <- c(1,2,5,7,10);
...
### Added by Mahsa
epsGrid <- seq(from=1/nGrid,to=1,length=nGrid);
...
if(k == numval){
    ...
    ##Added by mahsa
    data1 = data.frame(tab[,"eps"],tab[,"mse.eps"])
    write.table(data1, "mse_epsk20.txt", append = FALSE, sep = " ", dec = ".",
                row.names = FALSE, col.names = c("EPS","MSE"))
...
if(k == numval){
  ...
  ##Added by mahsa
  data2 = data.frame(tab[,"eps"],tab[,"T.eps"])
  write.table(data2, "thresh_epsk20.txt", append = FALSE, sep = " ", dec = ".",
              row.names = FALSE, col.names = c("EPS","T"))
} else {
